...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div x-data="unitForm()" x-init="init()" class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
        <form action="{{ route('organization.unit.update', $unit) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
...
<script>
    function unitForm() {
        return {
            basedOn: '{{ old('based_on', $unit->based_on ?? 'company') }}',
            selectedCompany: '{{ old('company_id', $unit->company_id) }}',
            selectedHolding: '{{ old('holding_id', $unit->holding_id) }}',
