    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-gray-600 dark:text-gray-400">
                Kelola data anak perusahaan
            </p>
        </div>

        <a href="{{ route('organization.subsidiary.create') }}"
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 px-6 py-4">
            ...
            <div class="relative">
                ...
                <input
                    ...
                    @input="resetPage"
                    type="text"
                    placeholder="Cari anak perusahaan..."
            ...
            <table class="w-full min-w-full">
                <thead>
                    <tr class="border-y border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
                        <th class="px-5 py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400">No</th>
                        ...
                        <th @click="sortBy('parent_name')" class="px-5 py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                            Induk Perusahaan
                        </th>
                        <th @click="sortBy('holding_name')" class="px-5 py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                            Holding
                        </th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <template x-for="(row, index) in paginated" :key="row.id">
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition">
                            ...
                            <td class="px-5 py-4 text-sm text-gray-500 dark:text-gray-400" x-text="row.holding_name"></td>
                            <td class="px-5 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a :href="'{{ url('organization/subsidiary') }}/' + row.id + '/edit'"
                                    ...
                                    <form :action="'{{ url('organization/subsidiary') }}/' + row.id" method="POST"
                                        @submit.prevent="if(confirm('Apakah Anda yakin ingin menghapus anak perusahaan ini?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    <template x-if="filtered.length === 0">
                        <tr>
                            <td colspan="5" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                                Belum ada data anak perusahaan ditemukan.
                            </td>
                        </tr>
                    </template>
                </tbody>
        ...
        get filtered() {
            ...
            if (this.search) {
                ...
                filteredData = filteredData.filter(d =>
                    ...
                    (d.holding_name && d.holding_name.toLowerCase().includes(q))
                );
            }
