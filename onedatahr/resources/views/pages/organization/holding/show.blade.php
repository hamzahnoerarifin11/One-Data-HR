...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="grid grid-cols-1 gap-6">
        ...
        <div x-data="companyList()" class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Daftar Perusahaan</h2>
                ...
                <table class="w-full min-w-full">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-900/50">
                            <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">No</th>
                            <th @click="sortBy('name')" class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                                Nama Perusahaan
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <template x-for="(company, index) in filtered" :key="company.id">
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition">
                                ...
                                <td class="px-5 py-4 text-sm font-medium text-gray-900 dark:text-white" x-text="company.name"></td>
                                ...
                                <td class="px-5 py-4 text-center">
                                    <a :href="'{{ url('organization/company') }}/' + company.id" class="inline-flex items-center justify-center rounded-lg bg-blue-50 p-2 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/40 transition mr-1" title="Lihat">
                        ...
                        <template x-if="filtered.length === 0">
                            <tr>
                                <td colspan="4" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                                    Tidak ada perusahaan yang ditemukan di bawah holding ini.
                                </td>
                            </tr>
                        </template>
                    </tbody>
            ...
            <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                <p class="text-xs text-gray-500 dark:text-gray-400" x-text="'Total: ' + filtered.length + ' Perusahaan'"></p>
            </div>
        </div>
    </div>
</div>
...
function companyList() {
    return {
        // Pastikan relasi company tersedia di model Holding
        data: @json($holding->company ?? []),
        search: '',
        sortCol: 'name',
        sortDir: 'asc',
