    ...
    <div x-data="pemberkasanTable()" class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <table class="w-full min-w-full text-left">
                <thead>
                    <tr class="border-y border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
                        <th @click="sortBy('nama')" class="px-5 py-3 text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600 transition">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <template x-for="row in paginated" :key="row.id">
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition">
                            <td class="px-5 py-4">
                                <p class="text-sm font-medium text-gray-900 dark:text-white" x-text="row.nama"></p>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <a :href="row.edit_url" class="inline-flex items-center justify-center rounded-lg bg-yellow-50 p-2 text-yellow-600 hover:bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-400 dark:hover:bg-yellow-900/40 transition">
                                    ...
                                    <form :action="row.delete_url" method="POST" @submit.prevent="if(confirm('Hapus data pemberkasan ini?')) $el.submit()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-red-50 p-2 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40 transition">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <div class="flex items-center gap-2">
                <button @click="prevPage" :disabled="page === 1" class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 dark:border-gray-700 dark:text-white disabled:opacity-50 transition hover:bg-gray-50 dark:hover:bg-gray-800">Prev</button>
                <template x-for="p in displayedPages" :key="p">
                    <button @click="goToPage(p)" :class="page === p ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 dark:border-gray-700 hover:bg-blue-500/[0.08] hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-500 dark:text-white'" class="px-3 py-1 text-sm rounded-lg border transition" x-text="p"></button>
                </template>
                <button @click="nextPage" :disabled="page === totalPages" class="rounded-lg border border-gray-300 px-3 py-1.5 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 dark:border-gray-700 dark:text-white disabled:opacity-50 transition hover:bg-gray-50 dark:hover:bg-gray-800">Next</button>
            </div>
        </div>
    </div>
</div>
