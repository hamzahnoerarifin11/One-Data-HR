...
@section('title','Tes Kompetensi Lolos')

@section('content')
<div class="px-4 py-6">
    <h2 class="text-xl font-semibold mb-4">Tes Kompetensi Lolos</h2>
...
document.getElementById('load-btn').addEventListener('click', async ()=>{
    ...
    const r = await fetch(`{{ route('rekrutmen.metrics.kompetensi') }}?${params.toString()}`, {credentials:'same-origin'});
    if(!r.ok) return alert('Gagal memuat');
    const data = await r.json();
    if(window.ApexCharts){ const labels = data.map(x=> x.nama_posisi); const values = data.map(x=> x.total); const options = { chart:{type:'bar',height:320}, series:[{name:'Kompetensi Lolos',data:values}], xaxis:{categories:labels} }; new ApexCharts(document.querySelector('#chart'), options).render(); }
