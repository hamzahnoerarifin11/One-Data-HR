...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
                Edit Kandidat Lanjut User
            </h1>
            <p class="mt-2 text-gray-600 dark:text-gray-400">
                Ubah data kandidat yang telah diserahkan ke User
            </p>
        </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-lg
        ...
        <form action="{{ route('rekrutmen.kandidat_lanjut_user.update', $data) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select id="kandidat_select" name="kandidat_id" required
                           class="dark:bg-dark-900 shadow-theme-xs h-11 w-full appearance-none rounded-lg border border-gray-300 bg-gray-100 px-4 py-2.5 pr-11 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 cursor-not-allowed" disabled>
                            <option value="{{ $data->kandidat_id }}">
                                {{ $data->kandidat->nama }}
                            </option>
                        </select>
                        {{-- Hidden input karena select disabled tidak mengirim value --}}
                        <input type="hidden" name="kandidat_id" value="{{ $data->kandidat_id }}">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input id="posisi_dilamar" value="{{ $data->kandidat->posisi?->nama_posisi }}" readonly
                ...
                <div>
                    ...
                    <input id="tanggal_interview_hr" value="{{ $data->tanggal_interview_hr }}" readonly
                        class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg
                               border border-gray-300 bg-gray-50 px-4 py-2.5
                               text-sm text-gray-800 dark:border-gray-700
                               dark:bg-gray-900 dark:text-white/90 cursor-not-allowed">
                ...
                <div>
                    ...
                    <input name="user_terkait" value="{{ $data->user_terkait }}" placeholder="Nama User" required
                        class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-400">
                        Tanggal Interview ASS
                    </label>
                    <div class="relative">
                    <input type="date" name="tanggal_interview_user_ass"
                        value="{{ $data->tanggal_interview_user_ass }}"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" onclick="this.showPicker()" />
                            <span class="absolute top-1/2 right-3.5 -translate-y-1/2 pointer-events-none">
                                <svg class="fill-gray-700 dark:fill-gray-400" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M4.33317 0.0830078C4.74738 0.0830078 5.08317 0.418794 5.08317 0.833008V1.24967H8.9165V0.833008C8.9165 0.418794 9.25229 0.0830078 9.6665 0.0830078C10.0807 0.0830078 10.4165 0.418794 10.4165 0.833008V1.24967L11.3332 1.24967C12.2997 1.24967 13.0832 2.03318 13.0832 2.99967V4.99967V11.6663C13.0832 12.6328 12.2997 13.4163 11.3332 13.4163H2.6665C1.70001 13.4163 0.916504 12.6328 0.916504 11.6663V4.99967V2.99967C0.916504 2.03318 1.70001 1.24967 2.6665 1.24967L3.58317 1.24967V0.833008C3.58317 0.418794 3.91896 0.0830078 4.33317 0.0830078ZM4.33317 2.74967H2.6665C2.52843 2.74967 2.4165 2.8616 2.4165 2.99967V4.24967H11.5832V2.99967C11.5832 2.8616 11.4712 2.74967 11.3332 2.74967H9.6665H4.33317ZM11.5832 5.74967H2.4165V11.6663C2.4165 11.8044 2.52843 11.9163 2.6665 11.9163H11.3332C11.4712 11.9163 11.5832 11.8044 11.5832 11.6663V5.74967Z" fill="" />
                ...
                <div>
                    ...
                    <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                        <select name="hasil_ass"
                            class="dark:bg-dark-900 shadow-theme-xs h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                            :class="isOptionSelected && 'text-gray-800 dark:text-white/90'">
                            <option value="">-- Pilih --</option>
                            <option value="Lolos" {{ $data->hasil_ass == 'Lolos' ? 'selected' : '' }}>Lolos</option>
                            <option value="Tidak Lolos" {{ $data->hasil_ass == 'Tidak Lolos' ? 'selected' : '' }}>Tidak Lolos</option>
                        </select>
                        <span
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-400">
                        Tanggal Interview ASM
                    </label>
                    <div class="relative">
                    <input type="date" name="tanggal_interview_user_asm"
                        value="{{ $data->tanggal_interview_user_asm }}"
                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" onclick="this.showPicker()" />
                            <span class="absolute top-1/2 right-3.5 -translate-y-1/2 pointer-events-none">
                                <svg class="fill-gray-700 dark:fill-gray-400" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd" clip-rule="evenodd" d="M4.33317 0.0830078C4.74738 0.0830078 5.08317 0.418794 5.08317 0.833008V1.24967H8.9165V0.833008C8.9165 0.418794 9.25229 0.0830078 9.6665 0.0830078C10.0807 0.0830078 10.4165 0.418794 10.4165 0.833008V1.24967L11.3332 1.24967C12.2997 1.24967 13.0832 2.03318 13.0832 2.99967V4.99967V11.6663C13.0832 12.6328 12.2997 13.4163 11.3332 13.4163H2.6665C1.70001 13.4163 0.916504 12.6328 0.916504 11.6663V4.99967V2.99967C0.916504 2.03318 1.70001 1.24967 2.6665 1.24967L3.58317 1.24967V0.833008C3.58317 0.418794 3.91896 0.0830078 4.33317 0.0830078ZM4.33317 2.74967H2.6665C2.52843 2.74967 2.4165 2.8616 2.4165 2.99967V4.24967H11.5832V2.99967C11.5832 2.8616 11.4712 2.74967 11.3332 2.74967H9.6665H4.33317ZM11.5832 5.74967H2.4165V11.6663C2.4165 11.8044 2.52843 11.9163 2.6665 11.9163H11.3332C11.4712 11.9163 11.5832 11.8044 11.5832 11.6663V5.74967Z" fill="" />
                ...
                <div>
                    ...
                    <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
                        <select name="hasil_asm"
                            class="dark:bg-dark-900 shadow-theme-xs h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                            :class="isOptionSelected && 'text-gray-800 dark:text-white/90'">
                            <option value="">-- Pilih --</option>
                            <option value="Lolos" {{ $data->hasil_asm == 'Lolos' ? 'selected' : '' }}>Lolos</option>
                            <option value="Tidak Lolos" {{ $data->hasil_asm == 'Tidak Lolos' ? 'selected' : '' }}>Tidak Lolos</option>
                        </select>
                        <span
                                    class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                                    <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
            ...
            <div class="mb-6">
                ...
                <textarea name="catatan" placeholder="Catatan Tambahan..." rows="3"
                    class="dark:bg-dark-900 shadow-theme-xs w-full rounded-lg
                           border border-gray-300 bg-transparent px-4 py-2.5
                           text-sm text-gray-800 dark:border-gray-700
                           dark:bg-gray-900 dark:text-white/90">{{ $data->catatan }}</textarea>
            </div>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="rounded-lg bg-brand-600 px-6 py-2 text-sm text-white hover:bg-brand-700 transition">
                    Update Data
                </button>
            </div>
