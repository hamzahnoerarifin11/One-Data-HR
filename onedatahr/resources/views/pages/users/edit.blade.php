    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="mt-2 text-gray-600 dark:text-gray-400">
                Perbarui data pengguna sesuai hak akses.
            </p>
        </div>
                ...
                <div x-show="orgScope !== 'all'" x-cloak>
                    ...
                    <select name="holding_id" x-model="holdingId"
                        ...
                        @foreach($holdings as $holding)
                            <option value="{{ $holding->id }}" :selected="holdingId == '{{ $holding->id }}'">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('userFormData', () => ({
            orgScope: @json(old('org_scope', $user->org_scope ?? 'all')),
            holdingId: @json(old('holding_id', $user->holding_id ?? '')),
            companyId: @json(old('company_id', $user->company_id ?? '')),
            divisionId: @json(old('division_id', $user->division_id ?? '')),
            departmentId: @json(old('department_id', $user->department_id ?? '')),
            unitId: @json(old('unit_id', $user->unit_id ?? '')),
