...
<div class="p-4 sm:p-6">
    ...
    <tr>
        ...
        @php
            // 1. DEFINISI HIERARKI (gunakan kunci yang umum / konsisten)
            $jabatanHierarchy = [
                'Direktur' => 1,
                'General Manager' => 2,
                'senior_manager' => 2,
            ...
            // 2. AMBIL DATA JABATAN & DIVISI STAFF (YANG AKAN DINILAI)
            // Prefer position->name (lebih konsisten), fallback ke kolom Jabatan jika kosong
            $staffJobName = $staff->pekerjaan->first()?->position?->name ?? $staff->pekerjaan->first()?->Jabatan ?? '';
            $staffLevel = 99;
            foreach ($jabatanHierarchy as $jab => $lvl) {
                if ($staffJobName && stripos($staffJobName, $jab) !== false) {
                    $staffLevel = $lvl;
                    break;
                }
            }
            $staffDivisi  = $staff->pekerjaan->first()?->division?->name ?? $staff->pekerjaan->first()?->divisi?->name ?? '';

            $canAssess = false; // Default: Tidak bisa menilai
            $reason    = '';    // Alasan kenapa tidak bisa

            // 3. LOGIKA UNTUK SENIOR MANAGER (GENERAL MANAGER)
            if (isset($issenior_manager) && $issenior_manager) {
                if ($staffLevel === 3) { // Level 3 adalah Manager
                    $canAssess = true;
                } else {
                    $canAssess = false;
                    $reason = ($staffLevel < 3) ? 'Level di atas atau setara' : 'Hanya bisa menilai level Manager';
                }
            }
            // 4. LOGIKA UNTUK MANAGER
            elseif (isset($isManager) && $isManager) {
                $managerJobName = $karyawan->pekerjaan->first()?->position?->name ?? $karyawan->pekerjaan->first()?->Jabatan ?? '';
                $managerLevel = 99;
                foreach ($jabatanHierarchy as $jab => $lvl) {
                    if ($managerJobName && stripos($managerJobName, $jab) !== false) {
                        $managerLevel = $lvl;
                        break;
                    }
                }

                // Selama level staff di bawah manager, mereka bisa dinilai.
                if ($staffLevel > $managerLevel) {
                    $canAssess = true;
                } else {
                    $canAssess = false;
                    $reason = 'Level bawahan setara atau lebih tinggi dari Anda.';
                }
            }
        @endphp
