            ...
            <table class="w-full min-w-full border-collapse">
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition">
                        ...
                        <td class="px-6 py-4 text-md text-gray-600 dark:text-gray-300">{{ $peserta->kelompok->nama_mentor ?? '-' }}</td>
                        <td class="px-6 py-4 text-right">
                            @can('editTempaPeserta')
                            <a href="{{ route('tempa.peserta.edit', ['peserta' => $peserta->id_peserta]) }}" class="inline-flex items-center justify-center rounded-lg bg-yellow-50 p-2 text-yellow-600 hover:bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-400 dark:hover:bg-yellow-900/40 transition" title="Edit">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            @endcan
                            @can('deleteTempaPeserta')
                            <form action="{{ route('tempa.peserta.destroy', ['peserta' => $peserta->id_peserta]) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
