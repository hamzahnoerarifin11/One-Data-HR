...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-gray-600 dark:text-gray-400">
                Daftar kelompok TEMPA yang tersedia
            </p>
        </div>
    ...
    @php
        ...
        $tableData = $kelompoks->map(fn($row) => [
            'id'                => $row->id_kelompok,
            'nama_kelompok'     => $row->nama_kelompok,
            'nama_mentor'       => $row->nama_mentor,
            'tempat'            => ucfirst($row->tempat) . ($row->tempat == 'cabang' && $row->keterangan_cabang ? ' ('.$row->keterangan_cabang.')' : ''),
            'ketua_tempa'       => $row->ketuaTempa?->name ?? '-',
            'jumlah_peserta'    => $row->pesertas()->count(),
            'show_url'          => route('tempa.kelompok.show', $row->id_kelompok),
            'edit_url'          => route('tempa.kelompok.edit', $row->id_kelompok),
            'delete_url'        => route('tempa.kelompok.destroy', $row->id_kelompok),
            'can_edit'          => auth()->user()->can('editTempaKelompok'),
            'can_delete'        => auth()->user()->can('deleteTempaKelompok'),
        ])->values();
    @endphp

    <div x-data="kelompokTable()" class="rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <table class="w-full min-w-full">
                <thead>
                    <tr class="border-y border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
                        <th @click="sortBy('nama_kelompok')" class="px-5 py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                            <div class="flex items-center gap-1">
                                Nama Kelompok
                                <svg :class="sortCol === 'nama_kelompok' ? (sortDir === 'asc' ? 'rotate-0' : 'rotate-180') : 'opacity-20'" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg>
                            </div>
                        </th>
                        <th @click="sortBy('nama_mentor')" class="px-5 py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                            <div class="flex items-center gap-1">
                                Nama Mentor
                                <svg :class="sortCol === 'nama_mentor' ? (sortDir === 'asc' ? 'rotate-0' : 'rotate-180') : 'opacity-20'" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg>
                            </div>
                        </th>
                        <th @click="sortBy('tempat')" class="px-5 py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                            <div class="flex items-center gap-1">
                                Tempat
                                <svg :class="sortCol === 'tempat' ? (sortDir === 'asc' ? 'rotate-0' : 'rotate-180') : 'opacity-20'" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg>
                            </div>
                        </th>
                        <th @click="sortBy('ketua_tempa')" class="px-5 py-3 text-left text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                            <div class="flex items-center gap-1">
                                Ketua TEMPA
                                <svg :class="sortCol === 'ketua_tempa' ? (sortDir === 'asc' ? 'rotate-0' : 'rotate-180') : 'opacity-20'" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg>
                            </div>
                        </th>
                        <th @click="sortBy('jumlah_peserta')" class="px-5 py-3 text-center text-sm font-medium text-gray-600 dark:text-gray-400 cursor-pointer hover:text-blue-600">
                            <div class="flex items-center justify-center gap-1">
                                Peserta
                                <svg :class="sortCol === 'jumlah_peserta' ? (sortDir === 'asc' ? 'rotate-0' : 'rotate-180') : 'opacity-20'" class="w-4 h-4 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/></svg>
                            </div>
                        </th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <template x-for="row in paginated" :key="row.id">
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition">
                            <td class="px-5 py-4 text-sm font-medium text-gray-900 dark:text-white" x-text="row.nama_kelompok"></td>
                            <td class="px-5 py-4 text-sm text-gray-500 dark:text-gray-400" x-text="row.nama_mentor"></td>
                            <td class="px-5 py-4 text-sm text-gray-500 dark:text-gray-400" x-text="row.tempat"></td>
                            <td class="px-5 py-4 text-sm text-gray-500 dark:text-gray-400" x-text="row.ketua_tempa"></td>
                            <td class="px-5 py-4 text-center text-sm font-bold text-gray-700 dark:text-gray-300">
                                <span class="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900/30 dark:text-blue-400" x-text="row.jumlah_peserta"></span>
                            </td>
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <template x-if="row.can_delete">
                                        <form :action="row.delete_url" method="POST"
                                              @submit.prevent="if(confirm('Apakah Anda yakin ingin menghapus kelompok ini?')) $el.submit()">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                    ...
                    <template x-if="filtered.length === 0">
                        <tr>
                            <td colspan="6" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                                Belum ada kelompok TEMPA ditemukan.
                            </td>
                        </tr>
                    </template>
                </tbody>
        ...
        get filtered() {
            ...
            if (this.search) {
                ...
                filteredData = filteredData.filter(d =>
                    d.nama_kelompok.toLowerCase().includes(q) ||
                    d.nama_mentor.toLowerCase().includes(q) ||
                    d.ketua_tempa.toLowerCase().includes(q) ||
                    d.tempat.toLowerCase().includes(q)
                );
            }
