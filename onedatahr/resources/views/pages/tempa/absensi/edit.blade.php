...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Data Absensi TEMPA</h1>
            <p class="mt-2 text-gray-600 dark:text-gray-400">Perbarui data absensi peserta TEMPA untuk satu tahun</p>
        </div>
    ...
    <form action="{{ route('tempa.absensi.update', $absensiModel->id_absensi) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-800 dark:bg-white/[0.03] p-6">
                ...
                <div>
                    ...
                    <select id="tahun_absensi" name="tahun_absensi"
                        ...
                        @for($year = date('Y') - 1; $year <= date('Y') + 1; $year++)
                            <option value="{{ $year }}" {{ $absensiModel->tahun_absensi == $year ? 'selected' : '' }}>{{ $year }}</option>
                        @endfor
                    </select>
                    <span
                                class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
            ...
            <div id="peserta-info" class="mt-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-800 dark:bg-white/[0.03] p-6">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
                Pilih status kehadiran untuk setiap minggu. Perubahan akan disimpan sebagai data terbaru.
            </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-3">
                    ...
                    @for($i = 1; $i <= 5; $i++)
                        @php
                            // Mengambil data dari JSON absensi yang ada di database (0-based indexing)
                            $currentValue = $absensiModel->absensi_data[$key][$i] ?? '';
                        @endphp
                        <div class="flex items-center justify-between">
                            ...
                            <select id="absensi_{{ $key }}_{{ $i }}" name="absensi[{{ $key }}][{{ $i }}]"
                                ...
                                <option value="hadir" {{ $currentValue == 'hadir' ? 'selected' : '' }}>Hadir</option>
                                <option value="tidak_hadir" {{ $currentValue == 'tidak_hadir' ? 'selected' : '' }}>Tidak Hadir</option>
                            </select>
                                        <span
                                            class="pointer-events-none absolute top-1/2 right-4 z-30 -translate-y-1/2 text-gray-700 dark:text-gray-400">
                                            <svg class="stroke-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-lg dark:border-gray-800 dark:bg-white/[0.03] p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative w-40 h-24 rounded-lg overflow-hidden border border-gray-200 flex">
                        <a href="{{ Storage::url($absensiModel->bukti_foto) }}" target="_blank" class="block">
                            <img src="{{ asset('storage/' . $absensiModel->bukti_foto) }}" alt="Bukti Foto" class="object-cover w-full h-full">
                        </a>
                    </div>
                </div>
                @endif
...
// Jalankan saat halaman di-load untuk mengisi data yang sudah ada
document.addEventListener('DOMContentLoaded', updatePesertaInfo);

// Jalankan saat select berubah
document.getElementById('id_peserta').addEventListener('change', updatePesertaInfo);
</script>
@endsection
